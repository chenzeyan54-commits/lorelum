...
/** Verify the raw ZCode Hook ABI through the compiled CLI. */
export async function verifyZcodeHookScenario(
  compiledBinary: string,
  fixture: InstalledPacksFixture,
  workingDirectory: string,
): Promise<void> {
  const successful = await runZcodeHook(compiledBinary, fixture.storageRoot, {
  ...
  const malformed = await runZcodeHook(compiledBinary, fixture.storageRoot, "{");
  assert.equal(malformed.exitCode, 0);
  assert.equal(malformed.stdout, '{"continue":true}\n');
  assert.match(malformed.stderr, /^lore hook zcode degraded: /);

  const unsupported = await runZcodeHook(compiledBinary, fixture.storageRoot, {
  ...
  const unavailableStore = join(workingDirectory, "unavailable-zcode-hook-store");
  await writeFile(unavailableStore, "not a directory\n");
  const unavailable = await runZcodeHook(compiledBinary, unavailableStore, {
    hook_event_name: "SessionStart",
  });
  assert.equal(unavailable.exitCode, 0);
  assert.equal(unavailable.stdout, '{"continue":true}\n');
  assert.match(unavailable.stderr, /^lore hook zcode degraded: /);
}
...
function runZcodeHook(
  ...
  return runProcess(
    [binaryPath, "hook", "zcode", "--store-root", storageRoot],
    60_000,
