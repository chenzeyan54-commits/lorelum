...
test("Cursor marketplace exposes the lorelum Plugin from the lorelum-plugins namespace", async () => {
  const [manifest, marketplace] = await Promise.all([
    readFile(join(import.meta.dir, "../.cursor-plugin/plugin.json"), "utf8").then(
      (content) => JSON.parse(content) as PluginManifest,
    ),
    readFile(join(import.meta.dir, "../../../../.cursor-plugin/marketplace.json"), "utf8").then(
  ...
  // Cursor discovers a Plugin's hooks/hooks.json automatically. A manifest
  // `hooks` entry means an inline hook config or an exact Hook-file path, so a
  // directory value would be diagnosed as an unreadable Hook file.
  expect(manifest.hooks).toBeUndefined();
  if (manifest.logo !== undefined) {
    const logo = await stat(join(import.meta.dir, "..", manifest.logo));
    expect(logo.isFile()).toBe(true);
  }
