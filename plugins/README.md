...
`lorelum/` is the current Codex Plugin, published as `lorelum@lorelum` and shown as **Lorelum for Codex**. It calls the public `lore` CLI for Pack metadata. It must not import Engine packages, read LocalStore files directly, or reproduce retrieval and ranking behavior.
