...
Read this reference only after a semantic `lore query ... --json` has returned `data.state: "preparing"` or an error. Do not use it as a preflight checklist before the first query. The injected Pack Catalog remains available while recovery is in progress; do not rerun `lore pack list --details --json` unless the catalog itself is missing or truncated.
...
- Before a host Agent starts a Lorelum-dependent long task, acquire a machine lease with `lore backend lease acquire --json`; retain its opaque `leaseId`, renew it before the returned expiry with `lore backend lease renew <leaseId> --json`, and release it in a `finally` path with `lore backend lease release <leaseId> --json`. This is host automation, not a user-facing setup step. If a lease cannot be maintained, do not attempt automatic Backend handoff.
