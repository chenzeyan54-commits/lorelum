...
The Backend and model are user-level resources. The explicit Store root selects Pack base data, while `--cache-root` selects derived query artifacts. Stop a test Backend with `bun packages/cli/src/main.ts backend stop` when the check is complete.
...
The candidate is copied to `packages/backend/.artifacts/native/embedding/<target>/` (`darwin-arm64` on macOS, `linux-x64` on Linux, `win32-x64` on Windows). The completed native runtime is shared only as a developer build cache: `~/Library/Caches/Lorelum/native/v1` on macOS, `$XDG_CACHE_HOME/lorelum/native/v1` (or `~/.cache/lorelum/native/v1`) on Linux, `%LOCALAPPDATA%\Lorelum\Cache\native\v1` on Windows. A matching second worktree verifies and copies that runtime instead of running CMake again. The cache does not belong to `~/.lorelum`, is not selected by `config.yaml`, and is never used as the backend runtime path; removing it merely makes the next `build:native` rebuild the candidate.

This remains a CPU-only build. macOS uses the pinned CMake download and an ARMv8.4 baseline; Linux uses the distribution CMake/toolchain and the generic x86-64 baseline (no AVX2/AVX-512 requirement, OpenMP disabled); Windows downloads the pinned CMake and WinLibs MinGW-w64 toolchain into the worktree's ignored `.cache/native-build/tools/` and keeps the same generic x86-64 baseline with a static MinGW runtime (`-static`, `-D_WIN32_WINNT=0x0A00`), so no compiler install is needed. The source archive, any pinned toolchain download, and CMake intermediate files remain in the invoking worktree's ignored `.cache/native-build/` and are only needed on a cache miss. See the [native build guide](../../native/embedding/README.md) for lifecycle checks and the release/source trust boundary.
...
For Store-only keyword retrieval, QueryService reuses a derived FTS5 SQLite index under the selected Store root across CLI processes. It binds that index to a verified effective revision, updates it from LocalStore's retained revision deltas, and rebuilds only when the index or its history cannot be trusted. ProjectContext keyword retrieval instead uses a content-addressed FTS5 artifact in the user cache. Canonical Store rows or project files remain the source of returned summaries; QueryService does not expose SQLite handles.

Semantic query is a separate Engine use case composed inside the local Backend with the fixed Profile and the already loaded model. It reads a content-addressed complete artifact or verified progress for the selected Store-only or ProjectContext snapshot, and returns `complete` or safely reduced `partial` coverage. CLI must route semantic requests through that Backend; Engine must not import the Backend client. See [Query](../cli/query.md) for lifecycle and CLI behavior.
