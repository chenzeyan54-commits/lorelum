...
for working on the current Codex and ZCode artifacts from a checkout; it does
not define their public identities or marketplace schemas.
...
Both artifacts use the released `lore` executable. The CLI owns the Hook ABI
...
```sh
bun test plugins/scripts plugins/codex/lorelum/scripts plugins/zcode/lorelum/scripts
