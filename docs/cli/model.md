...
显式开始模型准备和加载。普通 semantic query、index 和 install 也会在实际需要模型时自动开始或加入同一任务；`model load` 的用途是主动等待完整准备，或在自动下载失败后明确重试。未指定本地模型且缓存为空时，默认从 Lorelum 的 Hugging Face 仓库下载固定版本，无需手填 URL 或登录。可通过[embedding 配置](../configuration/embedding.md)指定镜像或关闭下载。HTTP 层立即接受共享任务，但 CLI 会轮询 status，直到 `ready`、`failed` 或被显式 unload。
