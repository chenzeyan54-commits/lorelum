...
The macOS CPU target is an M1-compatible ARMv8.4/FP16/dot-product baseline, with no i8mm and no compiled GPU backend. It is verified on M4, not yet on every Apple Silicon device. The Linux x64 CPU target is the AVX2 baseline: `GGML_SSE42/AVX/AVX2/BMI2/FMA/F16C` are on, while `GGML_NATIVE` and `GGML_OPENMP` stay off, so a Haswell-era (2013+) x86-64 CPU is required and expected to encode documents roughly an order of magnitude faster than the previous scalar baseline; AVX-512 and AVX-VNNI variants remain future work. The scalar baseline was built and verified on Ubuntu 24.04 (glibc 2.39) under WSL2, which does not by itself certify every Linux distribution; the AVX2 recipe needs one rebuilt, hash-refreshed artifact from the same toolchain before release. The Windows x64 target shares that AVX2 baseline with a static MinGW runtime; its scalar artifact was verified end to end on Windows 11 x64 (pack install, model load, semantic index, and semantic query) when the parent-liveness opt-in gate landed, and the AVX2 rebuild needs the same verification.
