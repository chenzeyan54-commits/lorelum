...
- [x] 4.2 在隔离 Store/cache/model roots 上运行 Store-only、普通目录和嵌套 ProjectContext end-to-end 场景：parent/child config inheritance、incremental Pack override、单条坏 Practice fallback、空模型 cache 下首次 query 自动启动下载/index、`0` 毫秒预算不产生 transport deadline、queryable partial、完整转 ready、相同语料 artifact/vector reuse、增量变更、rebuild、model unload/reload 与 cache prune recovery；这些场景 SHALL 作为仓库内可显式运行的 native smoke integration 保留，记录实际命令与观察结果。
